...
namespace gcsep
{
    public static class GCSEUtils
    {
        ...
        public static GCSENpcs CSE(this NPC npc)
            => npc.GetGlobalNPC<GCSENpcs>();
        public static GCSEPlayer CSE(this Player player)
            => player.GetModPlayer<GCSEPlayer>();
        ...
        public static void DisplayLocalizedText(string key, Color? textColor = null)
        {
            ...
            {
                ChatHelper.BroadcastChatMessage(NetworkText.FromKey(key, []), textColor.Value, -1);
            }
        }
    }
}