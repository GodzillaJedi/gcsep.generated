...
namespace gcsep.Thorium.Enchantments
{
    ...
    public class IridescentEnchant : BaseEnchant
    {
        ...
        public override void UpdateAccessory(Player player, bool hideVisual)
        {
            ...
            if (player.AddEffect<EqualizerEffect>(Item))
            {
                ModContent.GetInstance<Equalizer>().UpdateAccessory(player, hideVisual);
            }
            if (player.AddEffect<LifeQuartzShieldEffect>(Item))
            {
                ModContent.GetInstance<LifeQuartzShield>().UpdateAccessory(player, hideVisual);
            }
        }
        public class IridescentEffect : AccessoryEffect
        {
            ...
            {
                ModContent.GetInstance<IridescentHelmet>().UpdateArmorSet(player);
            }
        }
        public class EqualizerEffect : AccessoryEffect
        {
            public override Header ToggleHeader => Header.GetHeader<AlfheimForceHeader>();
            public override int ToggleItemType => ModContent.ItemType<IridescentEnchant>();
            public override bool MutantsPresenceAffects => true;
        }
        public class LifeQuartzShieldEffect : AccessoryEffect
        {
            public override Header ToggleHeader => Header.GetHeader<AlfheimForceHeader>();
            public override int ToggleItemType => ModContent.ItemType<IridescentEnchant>();
            public override bool MutantsPresenceAffects => true;
