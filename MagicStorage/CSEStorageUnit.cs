...
public class CSEStorageUnit : StorageUnit
{
    ...
    public override bool RightClick(int i, int j)
    {
        ...
        if (CSEStorageUnit.TryUpgrade(i, j))
        {
            return true;
        }
    ...
    private static bool TryUpgrade(int i, int j)
    {
        ...
        {
            CSEStorageUnit.SetStyle(i, j, 9);
            success = true;
        }
        else if (style == 9 && item.type == ModContent.ItemType<UpgradeAbominable>())
        {
            CSEStorageUnit.SetStyle(i, j, 11);
            success = true;
        }
        else if (style == 11 && item.type == ModContent.ItemType<UpgradeEternal>())
        {
            CSEStorageUnit.SetStyle(i, j, 13);
            success = true;
        }
        if (success)
        {
            if (!TileEntity.ByPosition.TryGetValue(new Point16(i, j), out var te) || !(te is TEGCSEPStorageUnit storageUnit))
            {
                return false;
            }
            ...
            Dust.NewDustPerfect(((TileEntity)(object)storageUnit).Position.ToWorldCoordinates(), DustID.PureSpray, Vector2.Zero, 0, Color.Green, 2f);
            return true;
        }
        return false;
    }
    ...
    public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
    {
        ...
        spriteBatch.Draw(ModContent.Request<Texture2D>("gcsep/MagicStorage/ShtunStorageUnit_Glow", (AssetRequestMode)2).Value, drawPos, frame, color);
    }
}