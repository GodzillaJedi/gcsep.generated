...
public class TEGCSEPStorageUnit : TEStorageUnit
{
    ...
    public override void DepositItem(Item toDeposit)
    {
        ...
        TEGCSEPStorageUnit.DepositToItemCollection(items, toDeposit, ShtunCapacity, out var hasChange);
    ...
    internal static bool DepositToItemCollection(List<Item> items, Item toDeposit, int capacity, out bool hasChange)
    {
        ...
        {
            if (StorageAggregator.CanCombineItems(toDeposit, item, true) && item.stack < item.maxStack)
            {
    ...
    public override Item TryWithdraw(Item lookFor, bool locked = false, bool keepOneIfFavorite = false)
    {
        ...
        if (!TEGCSEPStorageUnit.WithdrawFromItemCollection(items, lookFor, out var result, keepOneIfFavorite))
        {
            return result;
        }
        if (Main.netMode != 1)
        ...
        while (true)
        {
            if (k >= 0)
            {
                ...
                if (ItemData.Matches(lookFor, item))
                {
                    ...
                    if (result != null)
                    {
                        if (StorageAggregator.CanCombineItems(result, item, true))
                        {
                            goto IL_00e1;
                        }
    ...
    internal static void SwapItems(TEGCSEPStorageUnit unit1, TEGCSEPStorageUnit unit2)
    {
        TEGCSEPStorageUnit tEShtunStorageUnit = unit2;
    ...
    internal void PackItems()
    {
        ...
        items = TEGCSEPStorageUnit.Compact(items, out var didPack);
    ...
    internal bool FlattenFrom(TEGCSEPStorageUnit source, out List<Item> transferredItems)
    {
        ...
        TEGCSEPStorageUnit.AttemptItemTransfer(this, source, out transferredItems);
    ...
    internal static List<Item> Compact(IEnumerable<Item> items, out bool didPack)
    {
        ...
        foreach (Item item in items)
        {
            ...
            {
                if (!pack.IsAir && pack.stack < pack.maxStack && StorageAggregator.CanCombineItems(item, pack, true))
                {
    ...
    internal static void AttemptItemTransfer(TEGCSEPStorageUnit destination, TEGCSEPStorageUnit source, out List<Item> transferredItems)
    {
        ...
        for (int d = 0; d < destination.NumItems; d++)
        {
            ...
            for (int s = source.NumItems - 1; s >= 0; s--)
            {
                ...
                if (!src.IsAir && StorageAggregator.CanCombineItems(dest, src, true))
                {
    ...
    public override void NetReceive(BinaryReader trueReader)
    {
        ...
        if (opCount > 0)
        {
            if (TileEntity.ByPosition.TryGetValue(((TileEntity)this).Position, out var te) && te is TEGCSEPStorageUnit otherUnit)
            {
