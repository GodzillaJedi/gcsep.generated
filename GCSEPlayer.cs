...
using gcsep.Content.Items.Summons;
...
namespace gcsep
{
    public partial class GCSEPlayer : ModPlayer
    {
        ...
        public bool equippedGemTechEnchantment;
                ...
                for (int i = 0; i < 1000; i++)
                {
                        ...
                        && Main.projectile[i].owner == Player.whoAmI && Player.whoAmI == Main.myPlayer)
                    {
                        ...
                        if (!spawned && Main.projectile[i].wet && WorldSavingSystem.EternityMode && !NPC.AnyNPCs(NPCID.DukeFishron))
                        {
                            ...
                            GCSEUtils.DisplayLocalizedText("Music: True Masochism by ENNWAY", Color.White);
                        }
                    }
                }
            }
        ...
        public override void OnEnterWorld()
        {
            ...
            {
                Main.NewText(Language.GetTextValue($"Mods.gcsep.Message.NoRework"), Color.LimeGreen);
            }
            ...
            {
                Main.NewText(Language.GetTextValue($"Mods.gcsep.Message.NoBalancing"), Color.LimeGreen);
            }
            if (ModLoader.TryGetMod("InfernalEclipseAPI", out Mod _) || ModLoader.TryGetMod("WHummusMultiModBalancing", out Mod _))
            {
                Main.NewText(Language.GetTextValue($"Mods.gcsep.Message.IHateRogue"), Color.LimeGreen);
            }
            ...
            //{
            //    Main.NewText(Language.GetTextValue($"Mods.gcsep.Message.NoSoABardHealer1"), Color.Purple);
            //    Main.NewText(Language.GetTextValue($"Mods.gcsep.Message.NoSoABardHealer2"), Color.Purple);
            //}
        }
