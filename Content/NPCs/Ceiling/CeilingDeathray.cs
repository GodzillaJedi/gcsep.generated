...
namespace gcsep.Content.NPCs.Ceiling
{
    public class CeilingDeathray : BaseDeathray
    {
        ...
        public CeilingDeathray() : base(120) { }
        ...
        public override void AI()
        {
            ...
            if (Main.npc[ai1].active && Main.npc[ai1].type == ModContent.NPCType<CeilingOfMoonLordFace>())
            {
            ...
            (float)Projectile.width * Projectile.scale,
            (int x, int y) =>
            {
                DelegateMethods.CastLight(x, y); // Cast light at tile coordinates
                return false; // Continue processing points
            }
        );
        }
    }
