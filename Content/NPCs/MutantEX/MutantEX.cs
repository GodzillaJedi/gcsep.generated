...
using gcsep.Content.Items.Placeable;
using gcsep.Content.NPCs.MutantEX.Projectiles;
using gcsep.Content.NPCs.MutantEX.Projectiles.Calamity;
using gcsep.Content.NPCs.MutantEX.Projectiles.Fargo;
using gcsep.Content.NPCs.MutantEX.Projectiles.Siblings;
using gcsep.Systems;
using System;
...
namespace gcsep.Content.NPCs.MutantEX
{
    [AutoloadBossHead]
    public class MutantEX : ModNPC
    {
        ...
        {
            return GCSEConfig.Instance.AlternativeSiblings;
        }
        public override string Texture => $"gcsep/Content/NPCs/MutantEX/MutantEX{FargoSoulsUtil.TryAprilFoolsTexture}";
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            bestiaryEntry.Info.AddRange([
                ...
                new FlavorTextBestiaryInfoElement($"Mods.gcsep.Bestiary.{Name}")
            ]);
        }
        ...
        public override void SetDefaults()
        {
            ...
            if (FargoSoulsUtil.AprilFools)
                NPC.GivenName = Language.GetTextValue("Mods.gcsep.NPCs.MutantEX.DisplayNameApril");
        }
        ...
        public override void OnSpawn(IEntitySource source)
        {
            if (ModContent.TryFind(Mod.Name, "Monstrosity", out ModNPC modNPC))
            {
                ...
                if (n != -1 && n != Main.maxNPCs)
                {
                    ...
                    GCSEUtils.DisplayLocalizedText("Music: Axion by Xi", Color.White);
                }
            }
        ...
        public override void AI()
        {
            ...
            GCSEUtils.DisplayLocalizedText("ai[0] = " + NPC.ai[0].ToString() + "  ai[1] = " + NPC.ai[1].ToString() + "  ai[2] = " + NPC.ai[2].ToString() + "  ai[3] = " + NPC.ai[3].ToString() + "  localai[0] = " + NPC.localAI[0].ToString() + "  localai[1] = " + NPC.localAI[1].ToString() + "  localai[2] = " + NPC.localAI[2].ToString() + "  localai[3] = " + NPC.localAI[3].ToString());

            GCSENpcs.mutantEX = NPC.whoAmI;
            ...
            if (WorldSavingSystem.SwarmActive || forceDespawn || (!p.active || p.dead || Vector2.Distance(NPC.Center, p.Center) > 3000f) && NPC.localAI[3] > 0)
            {
                ...
                if (WorldSavingSystem.SwarmActive || forceDespawn || !p.active || p.dead || Vector2.Distance(NPC.Center, p.Center) > 3000f)
                {
                    ...
                    if (NPC.timeLeft == 1)
                    {
                        ...
                        if (NPC.position.Y < 0)
                            NPC.position.Y = 0;
                        if (FargoSoulsUtil.HostCheck && ModContent.TryFind("gcsep", "Monstrosity", out ModNPC modNPC) && !NPC.AnyNPCs(modNPC.Type))
                        {
        ...
        void EModeSpecialEffects()
        {
            ...
            if (!SkyManager.Instance["gcsep:MutantEX"].IsActive())
                SkyManager.Instance.Activate("gcsep:MutantEX");
        ...
        const string GFBLocPath = $"Mods.gcsep.NPCs.MutantEX.GFBText.";
        ...
        private void Nuke()
        {
            ...
            if (NPC.ai[1] > (float)(120))
            {
                ...
                {
                    Main.LocalPlayer.GetModPlayer<GCSEPlayer>().Screenshake = 2;
                }
                if (Main.netMode != 1)
                {
