...
namespace gcsep.Content.NPCs.MutantEX.HitPlayer
{
    internal class MonstrHealthPlayer : ModPlayer
    {
        ...
        {
            if (!FargoSoulsUtil.BossIsAlive(ref GCSENpcs.mutantEX, ModContent.NPCType<MutantEX>()) && !FargoSoulsUtil.BossIsAlive(ref EModeGlobalNPC.mutantBoss, ModContent.NPCType<MutantBoss>()))
            {
                HealthReduction = 0;
            }
        }
