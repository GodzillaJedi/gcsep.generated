...
namespace gcsep.Content.NPCs.MutantEX.Projectiles.Fargo
{
    public class MutantSphereRing : ModProjectile
    {
        ...
        {
            if (Main.getGoodWorld && base.Projectile.hostile && !base.Projectile.friendly && Main.LocalPlayer.active && !Main.LocalPlayer.dead && !Main.LocalPlayer.ghost && FargoSoulsUtil.BossIsAlive(ref GCSENpcs.mutantEX, ModContent.NPCType<MutantEX>()) && Main.npc[GCSENpcs.mutantEX].ai[0] == -5f && base.Projectile.Colliding(base.Projectile.Hitbox, Main.LocalPlayer.FargoSouls().GetPrecisionHurtbox()))
            {
