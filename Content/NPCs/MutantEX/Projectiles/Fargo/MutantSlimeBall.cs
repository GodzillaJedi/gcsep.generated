...
public class MutantSlimeBall : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if ((base.Projectile.localAI[1] += 1f) > 10f && FargoSoulsUtil.BossIsAlive(ref GCSENpcs.mutantEX, ModContent.NPCType<MutantEX>()) && Math.Sign(base.Projectile.Center.Y - Main.npc[GCSENpcs.mutantEX].Center.Y) == Math.Sign(base.Projectile.velocity.Y) && base.Projectile.Distance(Main.npc[GCSENpcs.mutantEX].Center) > 1200f + base.Projectile.ai[0])
        {
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        Texture2D texture2D13 = ModContent.Request<Texture2D>($"gcsep/Assets/ExtraTextures/MutantEX/MutantSlimeBall_{sheetClamped}", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
