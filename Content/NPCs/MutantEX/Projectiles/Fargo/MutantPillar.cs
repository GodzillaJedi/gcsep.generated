...
public class MutantPillar : ModProjectile
{
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        {
            Main.LocalPlayer.GetModPlayer<GCSEPlayer>().Screenshake = 30;
        }
        ...
        if (FargoSoulsUtil.BossIsAlive(ref GCSENpcs.mutantEX, ModContent.NPCType<MutantEX>()) && Main.npc[GCSENpcs.mutantEX].ai[0] == 19f)
        {
            fragmentDuration = (int)Main.npc[GCSENpcs.mutantEX].localAI[0];
        }
        float speed = 5.5f;
        for (int j = 0; j < 4; j++)
        {
