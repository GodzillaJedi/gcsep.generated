...
using static gcsep.SoA.Enchantments.ExitumLuxEnchant;
using static gcsep.SoA.Forces.FoundationsForce;
...
namespace gcsep.SoA.Enchantments
{
    ...
    public class FrosthunterEnchant : BaseEnchant
    {
        ...
        {
            player.AddEffect<FrostheadressEffect>(Item);
            player.AddEffect<FrosthunterEffect>(Item);
            if (player.AddEffect<DecreeEffect>(Item))
            {
                ModContent.GetInstance<DecreeCharm>().UpdateAccessory(player, hideVisual);
            }
        }

        public class FrostheadressEffect : AccessoryEffect
        {
            ...
            {
                ModContent.GetInstance<FrosthunterHeaddress>().UpdateArmorSet(player);
            }
        }
        public class DecreeEffect : AccessoryEffect
        {
            public override Header ToggleHeader => Header.GetHeader<FoundationsForceHeader>();
            public override int ToggleItemType => ModContent.ItemType<FrosthunterEnchant>();
        }
