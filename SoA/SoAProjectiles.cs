...
using static gcsep.SoA.Enchantments.QuasarEnchant;
    ...
    public class SoAProjectiles : GlobalProjectile
    {
        ...
        public override void AI(Projectile projectile)
        {
            ...
            if (projectile.type == ModContent.ProjectileType<TenebrisLink2>())
            {
                ...
                {
                    GCSEUtils.HomeInOnNPC(projectile, true, 1600, 10, 2);
                }
            }
            ...
            if (projectile.type == ModContent.ProjectileType<SpookGrenade>())
            {
                ...
                if ((projectile.ai[1] += 1f) >= 20f)
                {
                    GCSEUtils.HomeInOnNPC(projectile, true, 700, 10, 2);
                }
            }
            ...
            if (projectile.type == ModContent.ProjectileType<DesperatioBullet>())
            {
                ...
                {
                    GCSEUtils.HomeInOnNPC(projectile, true, 300, 10, 2);
                }
            }
        ...
        public override bool OnTileCollide(Projectile projectile, Vector2 oldVelocity)
        {
            ...
            {
                if (projectile.owner.ToPlayer().HasEffect<FlariumHelmEffect>() && Main.rand.NextFloat() < 0.15f && projectile.damage > 0 && !projectile.minion && projectile.tileCollide)
                {
        ...
        public override bool PreDraw(Projectile npc, ref Color lightColor)
        {
            if (npc.type == ModContent.ProjectileType<MutantBossProjectile>())
            {
                ...
                {
                    Texture2D suppTexture = ModContent.Request<Texture2D>("gcsep/Assets/ExtraTextures/MutantSupression", (AssetRequestMode)2).Value;
