...
namespace gcsep.Calamity
{
    ...
    public class WotGProj : GlobalProjectile
    {
        ...
        public override bool PreAI(Projectile projectile)
        {
            ...
            if (projectile.type == ModContent.ProjectileType<EmptinessSprayerGas>() && (GCSEUtils.ProjGetDistanceToNPC(projectile, targetMutantNPC) < 100 ||
                                                                                        GCSEUtils.ProjGetDistanceToNPC(projectile, targetMutant) < 100 ||
                                                                                        GCSEUtils.ProjGetDistanceToNPC(projectile, targetAbom) < 100 ||
                                                                                        GCSEUtils.ProjGetDistanceToNPC(projectile, targetAbomNPC) < 100 ||
                                                                                        GCSEUtils.ProjGetDistanceToNPC(projectile, targetDeviNPC) < 100 ||
                                                                                        GCSEUtils.ProjGetDistanceToNPC(projectile, targetDevi) < 100 ||
                                                                                        GCSEUtils.ProjGetDistanceToNPC(projectile, targetMonster) < 100 ||
                                                                                        GCSEUtils.ProjGetDistanceToNPC(projectile, targetMonsterNPC) < 100))
            {
                projectile.Kill();
            }
